...
import { Bell, Search, ChevronRight, LogOut, Plus, Menu, X } from 'lucide-react';
      ...
      <div className="px-4 sm:px-6 py-3 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-2 min-w-0">
          <button
            ...
            className="lg:hidden w-10 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition shrink-0"
          ...
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-sm text-slate-400 hidden md:inline">Operations</span>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className="md:hidden w-10 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition"
            aria-label="Toggle search"
          >
            {mobileSearchOpen ? <X className="w-4.5 h-4.5" /> : <Search className="w-4.5 h-4.5" />}
          </button>
          ...
          {canSubmit && (
            <>
              ...
              {/* Mobile icon-only new-report */}
              <button
                onClick={onNewReport}
                className="sm:hidden w-10 h-10 rounded-xl bg-linear-to-br from-indigo-500 to-violet-600 hover:from-indigo-600 hover:to-violet-700 text-white flex items-center justify-center shadow-md shadow-indigo-500/30 transition"
          ...
          <div className="hidden sm:flex items-center gap-2.5 pl-2 pr-1 py-1 rounded-xl border border-slate-200 bg-white">
            ...
            <div className="leading-tight pr-1 hidden md:block">
          ...
          {/* Mobile user chip — avatar + sign-out only */}
          <div className="sm:hidden flex items-center gap-1">
            <div
              className="w-9 h-9 rounded-lg bg-linear-to-br from-indigo-500 to-violet-600 text-white text-xs font-bold flex items-center justify-center"
              title={user?.fullName ?? '—'}
            >
              {initialsOf(user?.fullName)}
            </div>
            <button
              onClick={onLogout}
              className="w-10 h-10 rounded-xl bg-slate-100 hover:bg-rose-50 hover:text-rose-600 text-slate-500 flex items-center justify-center transition"
              aria-label="Sign out"
            >
              <LogOut className="w-4 h-4" />
            </button>
        ...
        <div className="md:hidden px-4 pb-3 -mt-1">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => onSearchChange(e.target.value)}
              placeholder="Search agent, location, summary..."
              className="w-full pl-10 pr-3 py-2.5 bg-slate-100 border border-transparent rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition"
            />
          </div>
        </div>
      )}
